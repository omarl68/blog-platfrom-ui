...
export class AdminUserManagementComponent implements OnInit {
  ...
  roleIdMapping: { [key: string]: string } = {
    ...
    '68569868a2f65688c5e0c965': 'reader',   // Reader role ID (from omar's user)
  };
  ...
  getRoleLabel(role: any): string {
    ...
    // Handle role as string (ID) or object
    let roleCode: string;
    if (typeof role === 'string') {
      // If role is a string (ID), map it to role code
      roleCode = this.roleIdMapping[role] || 'reader';
    } else {
